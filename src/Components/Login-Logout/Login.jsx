...
import { signInWithEmailAndPassword, GoogleAuthProvider, FacebookAuthProvider, signInWithPopup, signOut } from 'firebase/auth';
...
const Login = () => {
  ...
  return (
    <div className='h-screen grid place-items-center bg-center login-background'>
      <div className="bg-white text-center  lg:-mt-3 mx-auto grid grid-rows-5 lg:h-formHeight gap-2 p-4 rounded-lg lg:w-1/4 ">
        <img className="w-10 h-10 lg:w-20 rounded-full border-4 border-cyan-950 mx-auto lg:h-20" src={image} alt="motiva logo" />
        <h1 className='text-cyan-900 font-extrabold lg:text-xl'>Welcome back, {isLoggedIn ? 'Sign Out' : 'Sign In'}</h1>
        ...
        <div className="flex  flex-col justify-center gap-4">
          ...
          >
            <img src={google} className='-ml-4   w-5 bg-white rounded-full' sizes='' />
            <>Sign in with Google</>
          </button>
          <button
