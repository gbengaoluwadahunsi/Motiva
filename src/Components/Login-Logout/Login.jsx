...
const Login = () => {
  ...
  return (
    <div className='h-screen grid place-items-center bg-center login-background'>
      <div className="bg-white text-center mx-auto grid grid-rows-6 h-full gap-2 p-4 rounded-lg w-1/4 ">
          ...
          {({ isSubmitting }) => (
            <Form  className='row-span-4 bg-white grid grid-row-5 '>
              ...
              <div className=' w-full   bg-gray-100 rounded relative h-8 b lg:h-14'>
                <Field className=' px-4 h-full w-full bg-gray-100 rounded border-2 border-cyan-950 row-span-2 pr-10' type={showPassword ? 'text' : 'password'} name="password" placeholder="Password" />
                <div className='absolute top-2 lg:top-6 right-3'>
                  {showPassword ?
                    <FaEyeSlash className='cursor-pointer  text-cyan-950' onClick={() => setShowPassword(false)} /> :
                    <FaEye className='cursor-pointer text-cyan-950' onClick={() => setShowPassword(true)} />
                  }
                </div>
              </div>
              <ErrorMessage className='text-red-700 font-semibold' name="password" component="div" />
              ...
              <button className='bg-cyan-950 rounded text-sm lg:text-3xl h-8 lg:h-14 text-cyan-100 lg:font-extrabold row-span-1' type="submit" disabled={isSubmitting}>
                Sign In
              </button>
            </Form>
          )}
        </Formik>
        <span className='mx-auto font-bold text-cyan-950'>OR</span>
        <div className="flex  flex-col justify-center gap-4">
          ...
          <FaFacebook  className='-ml-4 lg:text-3xl'/><span className='text-sm'>Sign in with Google</span>
          </button>
          ...
          <FaFacebook  className='lg:text-3xl'/><span  className='text-sm'>Sign in with Facebook</span>
          </button>
        </div>
